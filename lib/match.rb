module TennisStats
  class Match
    attr_reader :sets, :date
    
    ##
    # Initialize a new Match. Takes a date (optional).
    #
    ...
    ##
    # Look up a Player by handle (handles are per-match, not an attribute of
    # the Player object).
    #
    def player(handle)
      @players[handle]
    ...
    def add_player(handle, player)
      @players.store handle, player
    end
    ...
    def report
      puts "Match on #{@date ? @date : '[unknown]'}: #{winner.name} def #{loser.name} (#{final_score})"
      puts ""
      %w[
        points_won
        winners
